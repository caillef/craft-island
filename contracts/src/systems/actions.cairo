...
mod actions {
    ...
    impl ActionsImpl of IActions<ContractState> {
        ...
        fn use_item(ref self: ContractState, x: u64, y: u64, z: u64) {
            let mut world = get_world(ref self);
            let player = get_caller_address();
            GameResultTrait::unwrap(use_item_internal(ref world, player, x, y, z, true));
        }
        ...
        fn inventory_move_item(ref self: ContractState, from_inventory: u16, from_slot: u8, to_inventory_id: u16, to_slot: u8) {
            let mut world = get_world(ref self);
            let player = get_caller_address();
            GameResultTrait::unwrap(inventory_move_item_internal(ref world, player, from_inventory, from_slot, to_inventory_id, to_slot, true));
        }
    ...
    fn try_use_item(ref world: dojo::world::storage::WorldStorage, player: ContractAddress, x: u64, y: u64, z: u64) -> bool {
        GameResultTrait::is_ok(@use_item_internal(ref world, player, x, y, z, false))
    }
    ...
    fn try_place_gatherable_resource(ref world: dojo::world::storage::WorldStorage, player: ContractAddress, x: u64, y: u64, z: u64, resource_id: u16) -> bool {
        GameResultTrait::is_ok(@place_gatherable_resource_internal(ref world, player, x, y, z, resource_id))
    }
    ...
    fn try_buy_single(ref world: dojo::world::storage::WorldStorage, player: ContractAddress, item_id: u16, quantity: u32) -> bool {
        ...
        GameResultTrait::is_ok(@buy_internal(ref world, player, item_ids.span(), quantities.span(), true))
    }
    ...
    fn try_start_processing(ref world: dojo::world::storage::WorldStorage, player: ContractAddress, process_type: u8, amount: u32) -> bool {
        GameResultTrait::is_ok(@start_processing_internal(ref world, player, process_type, amount, false))
    }
    ...
    // Internal implementation of use_item with GameResult - contains ALL original logic
    fn use_item_internal(ref world: dojo::world::storage::WorldStorage, player: ContractAddress, x: u64, y: u64, z: u64, check_lock: bool) -> GameResult<()> {
        if check_lock {
            ensure_not_locked(@world, player);
        }
        ...
        if item_type == 41 { // hoe
            ...
            if item_type == 0 {
                return GameResult::Err(GameError::InvalidItem(0));
            }
            ...
            if item_type < 32 {
                ...
                // World structures only in main island
                if player_data.current_space_id != 1 {
                    return GameResult::Err(GameError::InvalidTarget);
                }
                ...
                // Tools - do nothing when used on empty space
                return GameResult::Ok(());
            } else {
    ...
    fn place_gatherable_resource_internal(ref world: dojo::world::storage::WorldStorage, player: ContractAddress, x: u64, y: u64, z: u64, resource_id: u16) -> GameResult<()> {
        ...
        let is_seed = resource_id == 47 || resource_id == 51 || resource_id == 53;
        ...
        if is_seed {
            ...
            let block_below_pos: u8 = (x % 4 + (y % 4) * 4 + (below_z % 4) * 16).try_into().unwrap();
        ...
        if selected_item != resource_id || selected_item == 0 {
            return GameResult::Err(GameError::InvalidItem(resource_id));
        }

        if !inventory.remove_item(slot, 1) {
            return GameResult::Err(GameError::InsufficientItems((resource_id, 1)));
        }

        // Create the resource
        let new_resource = GatherableResource {
            island_owner: player_data.current_space_owner,
            island_id: player_data.current_space_id,
            chunk_id: chunk_id,
            position: position,
            resource_id: resource_id,
            planted_at: starknet::get_block_info().unbox().block_timestamp,
            next_harvest_at: if is_seed { starknet::get_block_info().unbox().block_timestamp + 30 } else { 0 },
            harvested_at: 0,
            max_harvest: if is_seed { 1 } else { 255 },
            remained_harvest: if is_seed { 1 } else { 255 },
            destroyed: false,
            tier: 0,
        };
    ...
    // Internal implementation of sell with GameResult - contains ALL original logic
    fn sell_internal(ref world: dojo::world::storage::WorldStorage, player: ContractAddress) -> GameResult<u32> {
    ...
    // Internal implementation of start_processing with GameResult - contains ALL original logic
    fn start_processing_internal(ref world: dojo::world::storage::WorldStorage, player: ContractAddress, process_type: u8, input_amount: u32, strict_validation: bool) -> GameResult<()> {
        let current_time = starknet::get_block_timestamp();

        // Check if player is already locked
        let processing_lock: ProcessingLock = world.read_model(player);
        if processing_lock.unlock_time > current_time {
        ...
        let process_type_enum = match process_type {
            ...
            5 => ProcessType::ProcessClay,
            _ => {
                return if strict_validation {
                    GameResult::Err(GameError::InvalidInput("Invalid process type"))
                } else {
                    GameResult::Err(GameError::InvalidInput("Invalid process type"))
                };
            }
        ...
        if process_type_enum == ProcessType::None {
            return GameResult::Err(GameError::InvalidInput("Invalid process type"));
        }

        if input_amount == 0 {
            return GameResult::Err(GameError::InvalidQuantity(input_amount));
        }
        ...
        // For strict mode: require exact amount, for lenient mode: use what's available
        let actual_input_amount = if strict_validation {
            if available_items < input_amount {
                return GameResult::Err(GameError::InsufficientItems((config.input_item.try_into().unwrap(), input_amount)));
            }
            input_amount
        } else {
            if available_items < input_amount { available_items } else { input_amount }
        };

        // Calculate batches to process based on actual input amount
        let batches_to_process = actual_input_amount / config.input_amount;
        ...
        // Calculate exact items to remove based on actual batches
    ...
    // Internal helper function for item prices (eliminates duplication)
    fn get_item_price_internal(item_id: u16) -> u32 {
    ...
    // Internal implementation of buy with GameResult - supports both single and batch purchases
    fn buy_internal(ref world: dojo::world::storage::WorldStorage, player: ContractAddress, item_ids: Span<u16>, quantities: Span<u32>, check_inventory_space: bool) -> GameResult<u32> {
        let mut player_data: PlayerData = world.read_model(player);

        // Validate input arrays have same length
        if item_ids.len() != quantities.len() {
            return GameResult::Err(GameError::InvalidInput("Arrays must have same length"));
        }
        if item_ids.len() == 0 {
            return GameResult::Err(GameError::InvalidInput("Must buy at least one item"));
        }
        ...
        loop {
            ...
            // Get price for each item using unified price logic
        ...
        let mut actual_cost = total_cost;
        i = 0;
        loop {
            if i >= item_ids.len() {
                break;
            ...
            if check_inventory_space {
                // For safe mode: try to add with overflow handling
                let mut hotbar: Inventory = world.read_model((player, 0));
                let mut inventory: Inventory = world.read_model((player, 1));
                ...
                // For unsafe mode: force add items (may overflow inventory)
    ...
    // Internal implementation of inventory_move_item with GameResult
    fn inventory_move_item_internal(ref world: dojo::world::storage::WorldStorage, player: ContractAddress, from_inventory: u16, from_slot: u8, to_inventory_id: u16, to_slot: u8, check_lock: bool) -> GameResult<()> {
        if from_inventory == to_inventory_id && from_slot == to_slot {
            return GameResult::Err(GameError::InvalidInput("Cannot move item to same slot"));
        }
        ...
        let success = if from_inventory == to_inventory_id {
