...
UDojoModel* ADojoHelpers::parseCraftIslandPocketPlayerDataModel(struct Struct* model)
{
    ...
    for (int k = 0; k < members->data_len; k++) {
        ...
        ConvertTyToUnrealEngineType(member, "current_island_owner", "felt252", Model->CurrentIslandOwner);
        ConvertTyToUnrealEngineType(member, "current_island_id", "u16", Model->CurrentIslandId);
    }
